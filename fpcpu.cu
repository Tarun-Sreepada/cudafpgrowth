...
TreeNode *makeNode(int item, int count, TreeNode *parent)
{
...
int findItem(dMap *map, int item)
{
...
void addNodeToKV(KV *keyVal, TreeNode *node)
{
...
void addItemMap(dMap *map, TreeNode *node)
{
...
void addChild(TreeNode *node, int item, int count, int &location, dMap *top)
{
...
void partialMinerInitialize(dMap *map, int minSup);

void partialMiner(KV *keyVal, int minSup)
{
    ...
    // partialMinerInitialize<<<map->numOfItems, 1>>>(map, minSup);
    partialMinerInitialize(map, minSup);
}
...
void partialMinerInitialize(dMap *map, int minSup)
{
    ...
    for (int tid = 0; tid < map->numOfItems; tid++)
    {
        ...
        if (count >= minSup)
        {
            ...
            // __syncthreads();
            // __threadfence_system();
            // printf("Pattern Count: %d\r", patCount);
...
void buildInitialTree(int *transactions, int *indexes, int numOfIndexes, int minSup)
{
    TreeNode *root = makeNode(-1, 0, NULL);
    dMap *top = makeMap();
    patCount = 0;
    ...
    // printf("Num of items in map: %d\n", top->numOfItems);
    // partialMinerInitialize<<<top->numOfItems,1>>>(top, minSup);
    partialMinerInitialize(top, minSup);
...
void initialTree(std::string fileName, int minimumSupport)
{
    ...
    // printf("\nFinished building initial tree\n");
